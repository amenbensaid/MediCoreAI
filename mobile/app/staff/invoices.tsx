...
import { staffApi, type StaffInvoice } from '@/src/api/staff';
...
const copy = {
  fr: {
    eyebrow: 'Facturation',
    title: 'Factures séances',
    subtitle: 'Consultez les factures créées depuis les séances et ouvrez le PDF.',
    total: 'Total',
    paid: 'Payé',
  ...
  en: {
    ...
    subtitle: 'Review invoices generated from sessions and open the PDF.',
    total: 'Total',
    paid: 'Paid',
...
export default function StaffInvoicesScreen() {
  ...
  const loadInvoices = useCallback(async (isRefresh = false) => {
    ...
    setError('');
    try {
      const response = await staffApi.getInvoices(token, { limit: 100 });
  ...
  const openPdf = async (invoice: StaffInvoice) => {
    const token = staffSession.getToken();
    if (!token) return;
    await WebBrowser.openBrowserAsync(buildApiUrl(`/invoice-pdf/${invoice.id}`, { token }));
  };
            ...
            <View key={invoice.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <View style={styles.cardTop}>
                <View style={styles.cardBody}>
                  ...
                  <Text style={[styles.patient, { color: colors.muted }]}>{invoice.patientName || '-'}</Text>
...
const styles = StyleSheet.create({
  ...
  patient: { fontSize: 13, fontWeight: '800', marginTop: 3 },
