...
export default function PatientLoginScreen() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const response = await authApi.patientLogin(email.trim(), password);
      patientSession.setSession(response.data.token, response.data.user, { rememberMe });
    ...
    >
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <Pressable
            onPress={() => router.push('/auth/patient-forgot-password' as never)}
          ...
          <View style={styles.footerRow}>
            ...
            <Pressable
              onPress={() => router.push({
                pathname: '/auth/patient-register',
...
const styles = StyleSheet.create({
  ...
  homeLink: { color: '#64748b', fontSize: 13, fontWeight: '800', marginTop: 15, textAlign: 'center' },
});
